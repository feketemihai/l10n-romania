...
class res_partner(models.Model):
    ...
    def _download_anaf_data(self):
        ...
        if bool(date.today() - modify):
            result = requests.get(
                'http://static.anaf.ro/static/10/Anaf/TVA_incasare/ultim_' + date.today().strftime('%Y%m%d') + '.zip')
        ...
        self._cr.execute(
            "COPY res_partner_anaf (id,vat,start_date, end_date, publish_date, operation_date, operation_type) FROM '%s' WITH DELIMITER '#' NULL '' " % file_path)
        return True
    ...
    def _check_vat_on_payment(self):
        ...
        if self.anaf_history:
            if len(self.anaf_history) > 1 and ctx.get('check_date', False):
                lines = self.env['res.partner.anaf'].search([('id', 'in', [rec.id for rec in self.anaf_history]), (
                    'start_date', '<=', ctx['check_date']), ('end_date', '>=', ctx['check_date'])])
    ...
    def _check_vat_subjected(self):
        ...
        if self.vat:
            vat_country, vat_number = self.vat[
                :2].lower(), self.vat[2:].replace(' ', '')
        if vat_number and vat_country and vat_country.upper() == 'RO':
            res = requests.get(
                'http://openapi.ro/api/companies/' + str(vat_number) + '.json')
