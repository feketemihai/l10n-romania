...
class stock_balance(osv.osv):
    ...
    _columns = {
        ...
        'qty_in': fields.float('Qty In', digits_compute=dp.get_precision('Product UoM'), readonly=True),
        'amount_in': fields.float('Amount In',digits_compute= dp.get_precision('Account'), readonly=True),

        'qty_out': fields.float('Qty Out', digits_compute=dp.get_precision('Product UoM'), readonly=True),
        'amount_out': fields.float('Amount Out',digits_compute= dp.get_precision('Account'), readonly=True),
...
SELECT 
 ...
 (
  ...
  FROM stock_move sm 
    ...
    WHERE ( sm.state  = 'done' ) 
     GROUP BY pt.categ_id, sm.product_id, pu.id, sm.location_id, sm.date, sm.company_id 
    UNION 
    SELECT min(- sm.id) AS id, 
                    date_trunc('month', sm.date) AS date, 
                  to_char(date_trunc('day', sm.date), 'YYYY') AS year, 
                    to_char(date_trunc('day', sm.date), 'MM') AS month, 
                    sm.location_dest_id AS location_id, pt.categ_id, sm.product_id, pu.id AS product_uom,
  ...
  WHERE ( sm.state   = 'done') 
        GROUP BY pt.categ_id, sm.product_id, pu.id, sm.location_dest_id, sm.date, sm.company_id) smg 
    GROUP BY smg.date, smg.year, smg.month, smg.categ_id, smg.product_id, smg.location_id, smg.product_uom, smg.company_id 
