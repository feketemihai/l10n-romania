...
class StockMove(models.Model):
    ...
    def _create_in_svl(self, forced_quantity=None):
        ...
        if self.env.context.get("standard") or not self.env.user.company_id.romanian_accounting:
    ...
    def _is_reception(self):
        ...
        it_is = (
            self.env.user.company_id.romanian_accounting and
    ...
    def _is_consumption(self):
        """ Este un conum de materiale in productie"""
        it_is = (
            self.env.user.company_id.romanian_accounting
            and self._is_out()
            and self.location_dest_id.usage == "production"
    ...
    def _is_usage_giving(self):
        ...
        it_is = (
            self.env.user.company_id.romanian_accounting and
            (
                self.location_dest_id.usage == "usage_giving"  and self._is_out() or
    ...
    def _get_accounting_data_for_valuation(self):
        ...
        company = self.env.user.company_id
        if company.romanian_accounting and self.product_id.categ_id.stock_account_change:
            location_from = self.location_id
            location_to = self.location_dest_id
