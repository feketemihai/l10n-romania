...
class product_category(models.Model):
    ...
    property_account_creditor_price_difference_categ = fields.Many2one('account.account',
...
class ProductTemplate(models.Model):
    ...
    def get_product_accounts(self, fiscal_pos=None):
        ...
        stock_input_acc = self.property_stock_account_input and self.property_stock_account_input.id or False
        if not stock_input_acc:
            stock_input_acc = self.categ_id.property_stock_account_input_categ and self.categ_id.property_stock_account_input_categ.id or False

        stock_output_acc = self.property_stock_account_output and self.property_stock_account_output.id or False
        if not stock_output_acc:
            stock_output_acc = self.categ_id.property_stock_account_output_categ and self.categ_id.property_stock_account_output_categ.id or False

        journal_id = self.categ_id.property_stock_journal and self.categ_id.property_stock_journal.id or False
        account_valuation = self.categ_id.property_stock_valuation_account_id and self.categ_id.property_stock_valuation_account_id.id or False
...
class stock_location(models.Model):
    ...
    merchandise_type = fields.Selection([("store", "Store"), ("warehouse", "Warehouse")], "Merchandise type",
                                        default="warehouse")
    property_stock_account_input_location = fields.Many2one('account.account',
                                                            string='Stock Input Account',
                                                            help="When doing real-time inventory valuation, counterpart journal items for all incoming stock moves will be posted in this account, unless "
                                                                 "there is a specific valuation account set on the source location. When not set on the product, the one from the product category is used.",
                                                            company_dependent=True)
    property_stock_account_output_location = fields.Many2one('account.account',
                                                             string='Stock Output Account',
                                                             help="When doing real-time inventory valuation, counterpart journal items for all outgoing stock moves will be posted in this account, unless "
                                                                  "there is a specific valuation account set on the destination location. When not set on the product, the one from the product category is used.",
                                                             company_dependent=True)
    property_account_creditor_price_difference_location = fields.Many2one('account.account',
                                                                          string="Price Difference Account",
                                                                          help="This account will be used to value price difference between purchase price and cost price.",
                                                                          company_dependent=True)
    property_account_income_location = fields.Many2one('account.account',
                                                       string="Income Account",
                                                       help="This account will be used to value outgoing stock using sale price.",
                                                       company_dependent=True)
    property_account_expense_location = fields.Many2one('account.account',
...
class stock_quant(models.Model):
    ...
    def _account_entry_move(self, move):
        ...
        quants: browse record list of Quants to create accounting valuation entries for.
                Unempty and all quants are supposed to have the same location id (thay already moved in)
        ...
        if (move.location_id.usage == 'internal' and move.location_id.merchandise_type == 'store') \
            ...
            if (move.location_id.usage != 'internal' or
                ...
                journal_id, acc_src, acc_dest, acc_valuation = move.with_context(ctx)._get_accounting_data_for_valuation()
                if acc_src and acc_dest and acc_src != acc_dest:
                    res += quants._create_account_move_line( move, acc_src, acc_dest, journal_id)
                ctx['type'] = 'reception_diff_vat'
                if move.location_id.usage != 'supplier':
                    journal_id, acc_src, acc_dest, acc_valuation = move.with_context(ctx)._get_accounting_data_for_valuation()
                    if acc_src and acc_dest and acc_src != acc_dest:
                        res += quants._create_account_move_line(  acc_src, acc_dest, journal_id)
        ...
        if not move.picking_id.notice and move.location_id.usage == 'internal' and move.location_dest_id.usage not in (
                'supplier', 'transit'):
            ...
            if move.location_dest_id.usage == 'usage_giving':
                ...
                if acc_src and acc_dest and ctx['type'] == 'usage_giving':
                    res += self._create_account_move_line( move, acc_src, acc_dest, journal_id, )
                # Change context to create account moves for cost of goods
                # delivered  (e.g. 607 = 371)
                ctx['type'] = 'delivery'
    ...
    # metoda definita local
    def _prepare_account_move_line(self, move, qty, cost, credit_account_id, debit_account_id):
        res = super(stock_quant, self)._prepare_account_move_line(move, qty, cost, credit_account_id, debit_account_id)
        ...
        if context.get('type', False) and context.get('type') in (
                'reception_diff', 'reception_diff_vat', 'delivery_diff', 'delivery_diff_vat'):
