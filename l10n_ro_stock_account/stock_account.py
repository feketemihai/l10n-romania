...
class stock_move(osv.Model):
    ...
    def create_account_move_lines(self, cr, uid, ids, context=None):
        ...
        for move in self.browse(cr, uid, ids):
            ...
            journal_id, acc_src, acc_dest, acc_valuation = quant_obj._get_accounting_data_for_valuation(cr, uid, move, context=context)
            acc_move_lines = []
            if move.state == 'done':
                for quant in move.quant_ids:
                    lines = quant_obj._account_entry_move(cr, uid, [quant], move, context=context)
                    if lines:
                        for line in lines:
                            line_vals = line[2]
    ...
    def _get_invoice_line_vals(self, cr, uid, move, partner, inv_type, context=None):
        ...
        # If it is a returned stock move, change quantity in invoice with minus (probably to be done in account_storno)
        if move.origin_returned_move_id:
...
class stock_quant(osv.Model):
    ...
    def _account_entry_move(self, cr, uid, quants, move, context=None):
        ...
        if move.picking_id and move.picking_id.notice and move.location_id.usage == 'internal' and move.location_dest_id.usage == 'customer':
            ...
            if acc_src and acc_dest and acc_src != acc_dest:
                res += self._create_account_move_line(cr, uid, quants, move, acc_src, acc_dest, journal_id, context=ctx)
            # Change context to create account moves for cost of goods sold  (e.g. 607 = 371)        
            ctx['notice'] = False
            ctx['type'] = 'delivery'
        
        # Change context to create account moves for cost of goods sold in case of refund (e.g. 371 = 607)                    
        ...
        if not move.picking_id.notice and move.location_id.usage == 'internal' and move.location_dest_id.usage not in ('supplier', 'transit'):
            # Change context to create account moves for cost of goods delivered  (e.g. 607 = 371)                    
            ctx['notice'] = False
            ctx['type'] = 'delivery'
            # Change context to create account moves for collected VAT in case of minus in inventory  (e.g. 635 = 4427)                    
            if move.location_dest_id.usage == 'inventory':
                ctx['type'] = 'inventory_vat'
                journal_id, acc_src, acc_dest, acc_valuation = self._get_accounting_data_for_valuation(cr, uid, move, context=ctx)
                if acc_src and acc_dest and acc_src != acc_dest:
                    res += self._create_account_move_line(cr, uid, quants, move, acc_src, acc_dest, journal_id, context=ctx)
                # Change context to create account moves for minus in inventory  (e.g. 607 = 371 with VAT collected based)
    ...
    def _prepare_account_move_line(self, cr, uid, move, qty, cost, credit_account_id, debit_account_id, context=None):
        ...
        # Calculate VAT base and amount for price differences and associate it to account move lines                    
        if context.get('type', False) and context.get('type') in ('reception_diff','reception_diff_vat','delivery_diff','delivery_diff_vat'):
            ...
            if list_price<=valuation_amount:
                ...
                valuation_amount = list_price * qty - currency_obj.round(cr, uid, move.company_id.currency_id, valuation_amount * qty)
                taxes = []
                if move.product_id.taxes_id:
                    taxes = self.pool.get('account.tax')._unit_compute(cr, uid, move.product_id.taxes_id[0], valuation_amount)
                if taxes:
                    tax = taxes[0]
                    if context.get('type') in ('reception_diff','reception_diff_vat'):
                        ...
                        if context.get('type') == 'reception_diff':
                            debit_line_vals['tax_code_id'] = self.pool.get('account.tax.code').browse(cr, uid, tax['base_code_id']).uneligible_tax_code_id and self.pool.get('account.tax.code').browse(cr, uid, tax['base_code_id']).uneligible_tax_code_id.id or False
                            ...
                            debit_line_vals['tax_code_id'] = self.pool.get('account.tax.code').browse(cr, uid, tax['tax_code_id']).uneligible_tax_code_id and self.pool.get('account.tax.code').browse(cr, uid, tax['tax_code_id']).uneligible_tax_code_id.id or False
                    ...
                    if context.get('type') in ('delivery_diff','delivery_diff_vat'):
                        ...
                        if context.get('type') == 'delivery_diff':
                            credit_line_vals['tax_code_id'] = self.pool.get('account.tax.code').browse(cr, uid, tax['base_code_id']).uneligible_tax_code_id and self.pool.get('account.tax.code').browse(cr, uid, tax['base_code_id']).uneligible_tax_code_id.id or False
                            ...
                            credit_line_vals['tax_code_id'] = self.pool.get('account.tax.code').browse(cr, uid, tax['tax_code_id']).uneligible_tax_code_id and self.pool.get('account.tax.code').browse(cr, uid, tax['tax_code_id']).uneligible_tax_code_id.id or False
        ...
        # Calculate VAT base and amount for minus in inventories and associate it to account move lines                    
        if context.get('type', False) and (context.get('type') == 'inventory_exp' or context.get('type') == 'inventory_vat'):
            ...
            if taxes:
                ...
                if context.get('type') == 'inventory_exp':
                    ...
                    credit_line_vals['tax_amount'] = tax['amount']
                    credit_line_vals['account_id'] = tax['account_collected_id']
                    debit_line_vals['debit'] = tax['amount'] > 0 and round(tax['amount'],2) or 0.00
                    debit_line_vals['credit'] = tax['amount'] < 0 and -1 * round(tax['amount'],2) or 0.00
                    credit_line_vals['credit'] = tax['amount'] > 0 and round(tax['amount'],2) or 0.00
                    credit_line_vals['debit'] = tax['amount'] < 0 and -1 * round(tax['amount'],2) or 0.00
    ...
    def _get_accounting_data_for_valuation(self, cr, uid, move, context=None):
        ...
        if context.get('type', False):
            ...
            if move_type == 'usage_giving':
                # Change the account to the usage giving one defined in company: usualy 8035
            ...
            if move_type == 'inventory_vat':
                ...
                # Change the accounts with the vat one (442700) and undeductible one defined in company (usualy 635000) to suit move: 635000 = 442700
                if move.product_id.taxes_id and move.product_id.taxes_id[0].account_collected_id:
                    acc_src = move.product_id.taxes_id[0].account_collected_id.id or False
            ...
            if move_type == 'reception_diff_vat':
                # Receptions in location with inventory kept at list price
                # Change the accounts with the uneligible vat one (442810) to suit move: 3xx = 442810
                if move.product_id.taxes_id and move.product_id.taxes_id[0].account_collected_id and move.product_id.taxes_id[0].account_collected_id.uneligible_account_id:
                    acc_src = move.product_id.taxes_id[0].account_collected_id.uneligible_account_id.id
                else:
                    acc_src = False
            if move_type == 'delivery_diff':
                # Deliveries from location with inventory kept at list price
                # Change the accounts with the price difference one (3x8) to suit move: 3x8 = 3xx
            ...
            if move_type == 'delivery_diff_vat':
                # Deliveries from location with inventory kept at list price
                # Change the accounts with the uneligible vat one (442810) to suit move: 442810 = 3xx
                if move.product_id.taxes_id and move.product_id.taxes_id[0].account_collected_id and move.product_id.taxes_id[0].account_collected_id.uneligible_account_id:
                    acc_dest = move.product_id.taxes_id[0].account_collected_id.uneligible_account_id.id
                else:
                    acc_dest = False
                    
        # If it is a notice, check if picking type is incoming or outgoing and replace the stock accounts with the payable / receivable notice accounts
    ...
    def _create_account_move_line(self, cr, uid, quants, move, credit_account_id, debit_account_id, journal_id, context=None):
        res = []
        for quant in quants:
            res += self._prepare_account_move_line(cr, uid, move, quant.qty, quant.cost, credit_account_id, debit_account_id, context=context)
        return res
    ...
    def get_account_move_lines(self, cr, uid, ids, context=None):
        ...
        for pick in self.browse(cr, uid, ids, context=context):
            ...
            for move in pick.move_lines:
                if move.acc_move_id:
                    acc_move_lines += [aml.id for aml in move.acc_move_id.line_id]
            if acc_move_lines != []:
                self.pool.get('account.move.line').write(cr, uid, acc_move_lines, {'stock_picking_id': pick.id})
        return True
        ...
        for inv in self.browse(cr, uid, ids, context=context):
            ...
            for move in inv.move_ids:
                if move.acc_move_id:
                    acc_move_lines += [aml.id for aml in move.acc_move_id.line_id]
            if acc_move_lines != []:
                self.pool.get('account.move.line').write(cr, uid, acc_move_lines, {'stock_inventory_id': inv.id})
        return True
        
    def post_inventory(self, cr, uid, inv, context=None):
        res = super(stock_inventory, self).post_inventory(cr, uid, inv, context=context)
