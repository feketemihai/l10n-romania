...
class account_compensation(osv.osv):
    ...
    def onchange_line_ids(self, cr, uid, ids, line_dr_ids, line_cr_ids, compensation_currency, context=None):
        ...
        #compute the field is_multi_currency that is used to hide/display options linked to secondary currency on the compensation
        is_multi_currency = False
        #loop on the compensation lines to see if one of these has a secondary currency. If yes, we need to see the options
        for compensation_line in line_dr_ids+line_cr_ids:
            line_id = compensation_line.get('id') and self.pool.get('account.compensation.line').browse(cr, uid, compensation_line['id'], context=context).move_line_id.id or compensation_line.get('move_line_id')
...
#    _rec_name = 'number'
    ...
    def recompute_compensation_lines(self, cr, uid, ids, partner_id, journal_id, currency_id, date, context=None):
        ...
        total_debit = 0.0
        account_type = None
        ids = move_line_pool.search(cr, uid, [('state','=','valid'), ('account_id.reconcile', '=', True), ('reconcile_id', '=', False), ('partner_id', '=', partner_id)], context=context)
        ...
        for line in account_move_lines:
            ...
            if line.currency_id and currency_id == line.currency_id.id:
                ...
                #always use the amount booked in the company currency as the basis of the conversion into the compensation currency
    ...
    def onchange_date(self, cr, uid, ids, date, currency_id, company_id, context=None):
        ...
        compensation_currency_id = currency_id or self.pool.get('res.company').browse(cr, uid, company_id, context=ctx).currency_id.id
    ...
    def unlink(self, cr, uid, ids, context=None):
        for t in self.read(cr, uid, ids, ['state'], context=context):
            if t['state'] not in ('draft', 'cancel'):
                raise osv.except_osv(_('Invalid Action!'), _('Cannot delete compensation(s) which are already opened or paid.'))
    ...
    def compensation_move_line_create(self, cr, uid, compensation_id, move_id, company_currency, current_currency, context=None):
        ...
        for line in compensation.line_ids:
            ...
            # this calls res_curreny.compute() with the right context, so that it will take either the rate on the compensation if it is relevant or will use the default behaviour
            ...
            if line.amount == line.amount_unreconciled:
                if not line.move_line_id:
                    raise osv.except_osv(_('Wrong compensation line'),_("The invoice you are willing to pay is not valid anymore."))
            ...
            if line.move_line_id:
                ...
                if line.move_line_id.currency_id and line.move_line_id.currency_id.id != company_currency:
                    ...
                    if line.move_line_id.currency_id.id == current_currency:
                        # if the compensation and the compensation line share the same currency, there is no computation to do
    ...
    def action_move_line_create(self, cr, uid, ids, context=None):
        ...
        for compensation in self.browse(cr, uid, ids, context=context):
            ...
            move_id = move_pool.create(cr, uid, self.account_move_get(cr, uid, compensation.id, context=context), context=context)
            # Get the name of the account_move just created
            name = move_pool.browse(cr, uid, move_id, context=context).name
            # Create one move line per compensation line where amount is not 0.0
            rec_list_ids = self.compensation_move_line_create(cr, uid, compensation.id, move_id, company_currency, current_currency, context)
    ...
    def _compute_balance(self, cr, uid, ids, name, args, context=None):
        ...
        for line in self.browse(cr, uid, ids, context=context):
            ...
            if not move_line:
                ...
                #always use the amount booked in the company currency as the basis of the conversion into the compensation currency
                res['amount_original'] = currency_pool.compute(cr, uid, company_currency, compensation_currency, move_line.credit or move_line.debit or 0.0, context=ctx)
                res['amount_unreconciled'] = currency_pool.compute(cr, uid, company_currency, compensation_currency, abs(move_line.amount_residual), context=ctx)
    ...
    def _currency_id(self, cr, uid, ids, name, args, context=None):
        ...
        for line in self.browse(cr, uid, ids, context=context):
            ...
            if move_line:
                ...
                res[line.id] = line.compensation_id.currency_id and line.compensation_id.currency_id.id or line.compensation_id.company_id.currency_id.id
        return res
