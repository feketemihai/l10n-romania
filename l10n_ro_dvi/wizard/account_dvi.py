...
class AccountInvoiceDVI(models.TransientModel):
    ...
    customs_commission = fields.Monetary(string='Customs Commission')
    ...
    def _prepare_custom_duty_product(self):
        ...
        return {
            ...
            'taxes_id': False,
         #   'supplier_texes_id':False,
            'company_id': False,
        }

    def _prepare_customs_commission_product(self):
        # todo: de completat contrul 447
    ...
    def get_custom_duty_product(self):
        product_id = self.env['ir.config_parameter'].sudo().get_param('dvi.custom_duty_product_id')
        return self.env['product.product'].browse(int(product_id)).exists()

    @api.model
    def get_customs_commission_product(self):
        product_id = self.env['ir.config_parameter'].sudo().get_param('dvi.customs_commission_product_id')
        return self.env['product.product'].browse(int(product_id)).exists()
    ...
    def default_get(self, fields_list):
        ...
        defaults['date'] = invoice.invoice_date
