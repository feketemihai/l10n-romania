...
class account_compensation(osv.osv):
    ...
    def _get_exchange_lines(
        ...
        ctx1 = context.copy()
        amount1 = round(
            currency_obj._get_conversion_rate(
                cr, uid, line.move_line_id.currency_id, line.move_line_id.company_id.currency_id, ctx1
            )
            * abs(line.amount),
            2,
        )
        if (
            datetime.strptime(line.move_line_id.date, "%Y-%m-%d").month
            == datetime.strptime(line.compensation_id.date, "%Y-%m-%d").month
        ):
            ...
            amount2 = round(
                currency_obj._get_conversion_rate(
                    cr, uid, line.move_line_id.currency_id, line.move_line_id.company_id.currency_id, ctx2
                )
                * abs(line.amount),
                2,
            )
        ...
        if line.account_id.type in ("receivable", "asset"):
            amount = (-1) * amount
        return super(account_compensation, self)._get_exchange_lines(
            cr, uid, line, move_id, amount, company_currency, current_currency, context=context
        )
