...
class account_compensation(osv.osv):
    ...
    def name_get(self, cr, uid, ids, context=None):
        ...
        if context is None:
            context = {}
        return [
            (r["id"], (r["number"] or _("Compensation")))
            for r in self.read(cr, uid, ids, ["number"], context, load="_classic_write")
        ]
    ...
    def onchange_line_ids(self, cr, uid, ids, line_dr_ids, line_cr_ids, compensation_currency, context=None):
        ...
        line_dr_ids = self.resolve_2many_commands(cr, uid, "line_dr_ids", line_dr_ids, context)
        line_cr_ids = self.resolve_2many_commands(cr, uid, "line_cr_ids", line_cr_ids, context)
        ...
        for compensation_line in line_dr_ids + line_cr_ids:
            line_id = (
                compensation_line.get("id")
                and self.pool.get("account.compensation.line")
                .browse(cr, uid, compensation_line["id"], context=context)
                .move_line_id.id
                or compensation_line.get("move_line_id")
            )
    ...
    def _get_journal_currency(self, cr, uid, ids, name, args, context=None):
        ...
        for compensation in self.browse(cr, uid, ids, context=context):
            res[compensation.id] = (
                compensation.journal_id.currency
                and compensation.journal_id.currency.id
                or compensation.company_id.currency_id.id
            )
        return res
    ...
    #    _rec_name = 'number'
    _track = {
        "state": {"account_compensation.mt_compensation_state_change": lambda self, cr, uid, obj, ctx=None: True,},
    }
    ...
    _columns = {
        ...
        "period_id": fields.many2one(
            "account.period", "Period", required=True, readonly=True, states={"draft": [("readonly", False)]}
        ),
        "narration": fields.text("Notes", readonly=True, states={"draft": [("readonly", False)]}),
        "currency_id": fields.function(
            _get_journal_currency,
            type="many2one",
        ...
        "move_ids": fields.related(
            "move_id", "line_id", type="one2many", relation="account.move.line", string="Journal Items", readonly=True
        ),
        ...
        "is_multi_currency": fields.boolean(
            ...
            help="Fields with internal purpose only that depicts if the compensation is a multi currency one or not",
        ),
    }
    _defaults = {
        ...
        "company_id": lambda self, cr, uid, c: self.pool.get("res.company")._company_default_get(
            cr, uid, "account.compensation", context=c
        ),
    }

    def onchange_journal_compensation(
        self, cr, uid, ids, line_ids=False, partner_id=False, journal_id=False, company_id=False, context=None
    ):
        """price
        ...
        vals = self.onchange_journal(cr, uid, ids, journal_id, line_ids, partner_id, company_id, context)
    ...
    def onchange_partner_id(self, cr, uid, ids, partner_id, journal_id, currency_id, date, context=None):
        ...
        vals = self.recompute_compensation_lines(cr, uid, ids, partner_id, journal_id, currency_id, date, context=ctx)
    ...
    def recompute_compensation_lines(self, cr, uid, ids, partner_id, journal_id, currency_id, date, context=None):
        ...
        # drop existing lines
        line_ids = ids and line_pool.search(cr, uid, [("compensation_id", "=", ids[0])])
        ...
        for line in account_move_lines:
            ...
            if line.currency_id and currency_id == line.currency_id.id:
                ...
                amount_unreconciled = currency_pool.compute(
                    cr, uid, company_currency, currency_id, abs(line.amount_residual), context=context_multi_currency
                )
    ...
    def onchange_date(self, cr, uid, ids, date, currency_id, company_id, context=None):
        ...
        compensation_currency_id = (
            currency_id or self.pool.get("res.company").browse(cr, uid, company_id, context=ctx).currency_id.id
        )
    ...
    def onchange_journal(self, cr, uid, ids, journal_id, line_ids, partner_id, date, company_id, context=None):
        ...
        vals["value"].update({"currency_id": currency_id, "period_id": period_id})
        if partner_id:
            res = self.onchange_partner_id(cr, uid, ids, partner_id, journal_id, currency_id, date, context)
            for key in res.keys():
        ...
        for t in self.read(cr, uid, ids, ["state"], context=context):
            if t["state"] not in ("draft", "cancel"):
                raise osv.except_osv(
                    _("Invalid Action!"), _("Cannot delete compensation(s) which are already opened or paid.")
                )
    ...
    def _sel_context(self, cr, uid, compensation_id, context=None):
        ...
        :rtype: dict
        """
        company_currency = self._get_company_currency(cr, uid, compensation_id, context)
        current_currency = self._get_current_currency(cr, uid, compensation_id, context)
        if current_currency != company_currency:
            context_multi_currency = context.copy()
            compensation = self.pool.get("account.compensation").browse(cr, uid, compensation_id, context)
    ...
    def account_move_get(self, cr, uid, compensation_id, context=None):
        ...
        if compensation.number:
            ...
            if not compensation.journal_id.sequence_id.active:
                raise osv.except_osv(
                    _("Configuration Error !"), _("Please activate the sequence of selected journal !")
                )
            c = dict(context)
            c.update({"fiscalyear_id": compensation.period_id.fiscalyear_id.id})
            name = seq_obj.next_by_id(cr, uid, compensation.journal_id.sequence_id.id, context=c)
        else:
    ...
    def _get_exchange_lines(
        self, cr, uid, line, move_id, amount_residual, company_currency, current_currency, context=None
    ):
        """
    ...
    def _convert_amount(self, cr, uid, amount, compensation_id, context=None):
        ...
        return currency_obj.compute(
            cr, uid, compensation.currency_id.id, compensation.company_id.currency_id.id, amount, context=context
        )

    def compensation_move_line_create(
        self, cr, uid, compensation_id, move_id, company_currency, current_currency, context=None
    ):
        """
        ...
        date = self.read(cr, uid, [compensation_id], ["date"], context=context)[0]["date"]
        ctx = context.copy()
        ctx.update({"date": date})
        compensation = self.pool.get("account.compensation").browse(cr, uid, compensation_id, context=ctx)
        ...
        for line in compensation.line_ids:
            ...
            if not line.amount and not (
                line.move_line_id
                and not float_compare(line.move_line_id.debit, line.move_line_id.credit, precision_digits=prec)
            ...
            amount = self._convert_amount(cr, uid, line.amount, compensation.id, context=ctx)
            # if the amount encoded in compensation is equal to the amount unreconciled, we need to compute the
            # currency rate difference
            if line.amount == line.amount_unreconciled:
                if not line.move_line_id:
                    raise osv.except_osv(
                        _("Wrong compensation line"), _("The invoice you are willing to pay is not valid anymore.")
                    )
            ...
            move_line = {
                ...
                "currency_id": line.move_line_id
                and (company_currency != line.move_line_id.currency_id.id and line.move_line_id.currency_id.id)
                or False,
                "quantity": 1,
                "credit": 0.0,
                "debit": 0.0,
            ...
            if (
                line.move_line_id
                and line.move_line_id.currency_id
                and not currency_obj.is_zero(cr, uid, line.move_line_id.currency_id, foreign_currency_diff)
            ):
    ...
    def _get_company_currency(self, cr, uid, compensation_id, context=None):
        ...
        """
        return (
            self.pool.get("account.compensation")
            .browse(cr, uid, compensation_id, context)
    ...
    def action_move_line_create(self, cr, uid, ids, context=None):
        ...
        move_line_pool = self.pool.get("account.move.line")
        for compensation in self.browse(cr, uid, ids, context=context):
            local_context = dict(context, force_company=compensation.journal_id.company_id.id)
            if compensation.move_id:
                continue
            company_currency = self._get_company_currency(cr, uid, compensation.id, context)
            current_currency = self._get_current_currency(cr, uid, compensation.id, context)
            ...
            move_id = move_pool.create(
                cr, uid, self.account_move_get(cr, uid, compensation.id, context=context), context=context
            )
            ...
            self.write(cr, uid, [compensation.id], {"move_id": move_id, "state": "posted", "number": name,})
            ...
            for rec_ids in rec_list_ids:
                if len(rec_ids) >= 2:
                    reconcile = move_line_pool.reconcile_partial(cr, uid, rec_ids, "auto")
        return True
...
class account_compensation_line(osv.osv):
    ...
    def _compute_balance(self, cr, uid, ids, name, args, context=None):
        ...
        for line in self.browse(cr, uid, ids, context=context):
            ...
            compensation_currency = (
                line.compensation_id.currency_id and line.compensation_id.currency_id.id or company_currency
            )
    ...
    def _currency_id(self, cr, uid, ids, name, args, context=None):
        ...
        for line in self.browse(cr, uid, ids, context=context):
            ...
            if move_line:
                ...
                res[line.id] = (
                    line.compensation_id.currency_id
                    and line.compensation_id.currency_id.id
                    or line.compensation_id.company_id.currency_id.id
                )
        return res
    ...
    _columns = {
        ...
        "partner_id": fields.related(
            "compensation_id", "partner_id", type="many2one", relation="res.partner", string="Partner"
        ),
        "amount": fields.float("Amount", digits_compute=dp.get_precision("Account")),
        "reconcile": fields.boolean("Full Reconcile"),
        "type": fields.selection([("dr", "Debit"), ("cr", "Credit")], "Dr/Cr"),
        "move_line_id": fields.many2one("account.move.line", "Journal Item", copy=False),
        "date_original": fields.related(
            "move_line_id", "date", type="date", relation="account.move.line", string="Date", readonly=1
        ),
        "date_due": fields.related(
            "move_line_id", "date_maturity", type="date", relation="account.move.line", string="Due Date", readonly=1
        ),
        ...
        "currency_id": fields.function(
            _currency_id, string="Currency", type="many2one", relation="res.currency", readonly=True
        ),
    }
    _defaults = {
        "name": "",
            ...
            res.update(
                {
                    ...
                    and move_line.currency_id.id
                    or move_line.company_id.currency_id.id,
                }
            )
        return {
            "value": res,
