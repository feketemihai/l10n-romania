...
class hr_employee_related(models.Model):
    ...
    def _validate_relation(self):
        if self.relation_type and self.relation:
            if self.relation_type in ("coinsured", "both") and not self.relation in ("husband", "wife", "parent"):
    ...
    relation_type = fields.Selection(
        [("in_care", "In Care"), ("coinsured", "Coinsured"), ("both", "Both")],
...
class hr_employee(models.Model):
    ...
    def _number_personcare(self):
        self.person_in_care = self.person_related.search_count(
            [("relation_type", "in", ("in_care", "both")), ("employee_id", "=", self.id),]
        )
    ...
    def _ssnid_birthday_gender(self):
        if self.ssnid and self.country_id and "RO" in self.country_id.code.upper():
            ...
            if self.ssnid[0] in "1357":
                ...
                gender = "female"
            self.write({"gender": gender, "birthday": bday, "place_of_birth": bp})
    ...
    def _return_medic_exam_expiring(self):
        return self.search([("active", "=", True), ("medic_exam", ">=", self._medic_exam_expires()),])
    ...
    person_related = fields.One2many("hr.employee.related", "employee_id", "Related Persons")
    person_in_care = fields.Integer(
        string="No of persons in care", compute="_number_personcare", help="Number of persons in care"
    )
