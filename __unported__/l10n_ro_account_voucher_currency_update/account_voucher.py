...
class account_voucher(osv.osv):
    ...
    def _get_exchange_lines(
        ...
        if line.account_id.type in ("receivable", "asset"):
            amount = (-1) * amount
        return super(account_voucher, self)._get_exchange_lines(
            cr, uid, line, move_id, amount, company_currency, current_currency, context=context
        )

    def voucher_move_line_create(
        self, cr, uid, voucher_id, line_total, move_id, company_currency, current_currency, context=None
    ):
        """
        ...
        ctx.update(
            {
                ...
                "voucher_special_currency": voucher.payment_rate_currency_id
                and voucher.payment_rate_currency_id.id
                or False,
            }
        )
                ...
                if line.amount != line.amount_unreconciled:
                    if not line.move_line_id:
                        raise osv.except_osv(
                            _("Wrong voucher line"), _("The invoice you are willing to pay is not valid anymore.")
                        )
