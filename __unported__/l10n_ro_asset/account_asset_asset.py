...
class account_asset_category(models.Model):
    ...
    prorata = fields.Boolean(
        "Prorata Temporis",
        help="Indicates that the first depreciation entry for this asset have to be done from the purchase date instead of first day of next month.",
    )
    ...
    method_number_min = fields.Integer(
        "Minimum Number of Depreciations",
        help="The minimum number of depreciations of your asset - expressed in months",
    )
    method_number = fields.Integer(
        "Maximum Number of Depreciations",
        help="The maximum number of depreciations of your asset (taken by default) \
        - expressed in months",
    )
    ...
    sequence_id = fields.Many2one(
        "ir.sequence",
        "Inventory Sequence",
        help="This field contains the information related to the inventory numbering \
       of the assets from this category.",
        copy=False,
    )
...
class account_asset_asset(models.Model):
    ...
    def _amount_residual(self):
        self._cr.execute(
            ...
            FROM
                account_move_line l
            WHERE
                l.asset_id = %s GROUP BY l.asset_id """,
            (self.id,),
        )
    ...
    purchase_date = fields.Date(
        "Depreciation Start Date", required=True, readonly=True, states={"draft": [("readonly", False)]}
    )
    prorata = fields.Boolean(
        "Prorata Temporis",
        readonly=True,
    ...
    product_id = fields.Many2one(
        "product.product", string="Product", required=True, readonly=True, states={"draft": [("readonly", False)]}
    )
    asset_type = fields.Selection(
        [("none", "None"), ("fixed", "Fixed Assets"), ("service", "Services Assets")],
    ...
    def _check_method_number(self):
        if (self.method_time == "number") and (
            (self.method_number < self.category_id.method_number_min)
            or (self.method_number > self.category_id.method_number)
        ):
...
class account_asset_reevaluation(models.Model):
    ...
    type = fields.Selection(
        [("reevaluation", "Reevaluation"), ("modernization", "Modernization"), ("cassation", "Cassation")],
        string="Reevaluation Type",
        required=True,
        readonly=True,
        states={"draft": [("readonly", False)]},
        store=True,
    ...
    state = fields.Selection(
        [("draft", "Draft"), ("done", "Done")], string="Reevaluation State", required=True, store=True, default="draft"
    )
    date = fields.Date(
        "Reevaluation Date", readonly=True, states={"draft": [("readonly", False)]}, default=time.strftime("%Y-%m-%d")
    )
    ...
    def create_move(self):
        ...
        if not self.asset_id.company_id.property_asset_reevaluation_account_id:
            raise Warning(
                _(
                    "You need to configure the Asset Reevaluation Account in the Romanian Accounting Page of your company."
                )
            )
        move_vals = {
        ...
        move_line_obj.create(
            {
                ...
                "account_id": amount > 0
                and self.asset_id.company_id.property_asset_reevaluation_account_id.id
                or self.asset_id.category_id.account_asset_id.id,
                "debit": 0.0,
                "credit": abs(amount),
