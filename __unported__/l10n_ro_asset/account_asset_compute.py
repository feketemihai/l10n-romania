    ...
    def validate(self, cr, uid, ids, context=None):
        ...
        for asset in self.browse(cr, uid, ids, context=context):
            if asset.asset_type == "fixed" and not asset.inventory_number:
                asset.write(
                    {
                        "inventory_number": obj_sequence.next_by_id(
        ...
        if i == undone_dotation_number:
            ...
            if asset.method == "linear":
                ...
                if asset.prorata:
                    ...
                    if i == 1:
                        amount = round(
                            (amount_to_depr / asset.method_number)
    ...
    def compute_depreciation_board(self, cr, uid, ids, context=None):
        ...
        for asset in self.browse(cr, uid, ids, context=context):
            if asset.value_residual == 0.0:
                continue
            posted_lines = depreciation_lin_obj.search(
                cr, uid, [("asset_id", "=", asset.id), ("move_check", "=", True)], order="depreciation_date desc"
            )
            posted_depreciation_line_ids = []
            reevaluated_amount = 0.00
            for line in asset.reevaluation_ids:
                reevaluated_amount += line.diff_value
            old_posted_depreciation_line_ids = depreciation_lin_obj.search(
                cr, uid, [("asset_id", "=", asset.id), ("move_check", "=", True)], order="depreciation_date desc"
            )
            ...
            if len(posted_depreciation_line_ids) > 0:
                ...
                if old_posted_depreciation_line_ids:
                    last_depreciation_date = datetime.strptime(
                        depreciation_lin_obj.browse(
                            cr, uid, old_posted_depreciation_line_ids[0], context=context
                        ).depreciation_date,
                        "%Y-%m-%d",
                    )
                    depreciation_date = last_depreciation_date + relativedelta(months=+asset.method_period)
                    ...
                    if residual_amount != 0:
                        vals = {
                            ...
                            "depreciated_value": round(
                                (asset.purchase_value - asset.salvage_value) - residual_amount + reevaluated_amount, 2
                            ),
