...
class account_asset_asset(models.Model):
    ...
    picking_id = fields.Many2one("stock.picking", related="stock_move_id.picking_id", string="Stock Reception")
    prodlot_id = fields.Many2one("stock.production.lot", string="Production Lot")
    inventory_number = fields.Char(
        string="Inventory Number", store=True, readonly=True, states={"draft": [("readonly", False)]}, copy=False
    )
    inventory_value = fields.Float(
        string="Inventory Value",
        related="stock_move_id.price_unit",
        store=True,
        readonly=True,
...
class stock_move(osv.osv):
    ...
    _columns = {
        "location_dest_asset": fields.related(
            "location_dest_id", "asset_location", type="boolean", string="Destination Asset Location", readonly="1"
        ),
        "location_asset": fields.related(
            "location_id", "asset_location", type="boolean", string="Source Asset Location", readonly="1"
        ),
        "asset_category_id": fields.many2one(
            "account.asset.category", "Asset Category", domain=[("asset_type", "=", "fixed")]
        ),
    ...
    def onchange_asset_category_id(self, cr, uid, ids, asset_category_id=False, pick_type_id=False, context=None):
        ...
        if pick_type_id:
            ...
            dest_location_id = (
                pick_type.warehouse_id
                and pick_type.warehouse_id.wh_asset_loc_id
                and pick_type.warehouse_id.wh_asset_loc_id.id
                or False
            )
                ...
                for asset in move.asset_ids:
                    if asset.state != "draft":
                        raise except_orm(
                            _("Error!"),
                            _(
                                "You cannot cancel a stock move which doesn't have assets in draft state. You need to reset to draft the asset with name %s."
                                % asset.name
                            ),
                        )
        ...
        for move in self.browse(cr, uid, ids, context=context):
            if move.asset_id:
                ...
                if move.product_id.type == "consu":
                    ...
                    main_domain[move.id] = [
                        ("history_ids", "in", move.asset_id.stock_move_id.id),
                ...
                for ops in operations:
                    ...
                    for record in ops.linked_move_operation_ids:
                        ...
                        if move.id in main_domain:
                            ...
                            ).get_specific_domain(cr, uid, record, context=context)
                            qty = record.qty
                            if qty:
