...
class account_asset_asset(models.Model):
    ...
    invoice_id = fields.Many2one(
        "account.invoice",
        related="inv_line_id.invoice_id",
        string="Invoice",
        readonly=True,
        ...
        for line in lines:
            if line.asset_category_id:
                ...
                if line.asset_category_id.prorata:
                    ...
                    new_date = datetime.strptime(line.invoice_id.date_invoice, "%Y-%m-%d") + relativedelta(
                        day=1, months=+1
                    )
                vals = {
                    "name": line.name,
                    ...
                    for asset in inv_line.asset_ids:
                        if asset.state != "draft":
                            raise except_orm(
                                _("Error!"),
                                _(
                                    "You cannot cancel an invoice which doesn't have assets in draft state. You need to reset to draft the asset with name %s."
                                    % asset.name
                                ),
                            )
