...
class d394_report(osv.osv_memory):
    ...
    def _get_datas(self, cr, uid, ids, context=None):
        ...
        if data_company.email:
            xmldict.update(
                {"mail": data_company.email, "mailFisc": data_company.email,}
            )
        else:
            xmldict.update(
                {"mail": user.partner_id.email or "-", "mailFisc": user.partner_id.email or "-",}
            )
                    ...
                    if not inv.fiscal_receipt:
                        ...
                        if inv.fiscal_position and ("Taxare Inversa" in inv.fiscal_position.name):
                            ...
                            for tax_line in inv.tax_line:
                                ...
                                if any(i in tax_line.name for i in ("24", "20", " 9", " 5", " 0")):
                                    baza += (
                                        currency_obj.compute(
                                            cr,
                                            uid,
                ...
                if codes:
                    ...
                    for key in cer.iterkeys():
                        if cer[key]["baza"] != 0:
                            cereals.append(
                                {
                                    "code": self.pool.get("report.394.code").browse(cr, uid, key).name,
                        ...
                        if inv.partner_id.id == cui[key]:
                            if inv.fiscal_position and ("Taxare Inversa" in inv.fiscal_position.name):
                                for line in inv.invoice_line:
                                    if line.product_id.d394_id:
                                        code = line.product_id.d394_id.id
                                        prod_taxes = line.product_id.supplier_taxes_id
                                        if code in codes:
                                            cer[code]["baza"] += (
                                                currency_obj.compute(
                                                    cr,
...
bazaCc="%(bazaCc)s" tvaCc="%(tvaCc)s"/>""" % (
        ...
        model_data_ids = mod_obj.search(
            cursor, user, [("model", "=", "ir.ui.view"), ("name", "=", "view_d394_save")], context=context
        )
