    ...
    def _get_inv_number(self):
        ...
        for inv in self:
            if inv.type in ('out_invoice', 'out_refund') or inv.journal_id.sequence_type in ('autoinv1', 'autoinv2'):
                ...
                inv_serie = inv_seq._interpolate(inv_seq.prefix,
        ...
        for inv in self:
            ...
            if inv.type in ('out_invoice', 'out_refund'):
                ...
                if not partner.is_company and inv.origin_type:
                    oper_type = 'N'
                elif inv.fiscal_position and (('Taxare Inversa' in inv.fiscal_position.name) or \
                    ...
                    if partner.country_id and partner.country_id.id == country_ro.id:
                        ...
                        oper_type = 'C'
                elif not inv.fiscal_position or (inv.fiscal_position and
    ...
    def _get_tax_ids(self):
        for inv in self:
            ...
            if not inv.fiscal_position or (
                    ('National' in inv.fiscal_position.name) or ('Invers' in inv.fiscal_position.name) or (
                        ('Scutit' in inv.fiscal_position.name) and inv.partner_type in ('1', '2'))):
