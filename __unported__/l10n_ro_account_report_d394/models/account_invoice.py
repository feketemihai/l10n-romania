...
ORIGIN_TYPE = [("1", "Invoice"), ("2", "Slip"), ("3", "Trading Book"), ("4", "Contract")]
...
class AccountInvoice(models.Model):
    ...
    def _get_inv_number(self):
        ...
        for inv in self:
            if inv.type in ("out_invoice", "out_refund") or inv.journal_id.sequence_type in ("autoinv1", "autoinv2"):
                inv_seq = inv.journal_id.sequence_id
                ctx = self._context.copy()
                ctx["fiscalyear_id"] = inv.period_id.fiscalyear_id.id
                inv_serie = inv_seq._interpolate(
                    inv_seq.prefix, inv_seq.with_context(ctx)._interpolation_dict_context()
                )
                inv.inv_serie = regex.sub("", inv_serie)
                if inv.move_name:
                    inv.inv_number = int(regex1.sub("", inv.move_name.replace(inv_serie, "")))
            else:
                if inv.supplier_invoice_number and regex1.sub("", inv.supplier_invoice_number):
                ...
                if val:
                    ...
                    if inv_serie:
                        inv.inv_number = int(regex1.sub("", val.replace(inv_serie, "")))
                    else:
                        inv.inv_number = int(regex1.sub("", val))
        return True
        ...
        for inv in self:
            ...
            if inv.type in ("out_invoice", "out_refund"):
                if inv.fiscal_position and ("Taxare Inversa" in inv.fiscal_position.name):
                    oper_type = "V"
                elif not inv.fiscal_position or (inv.fiscal_position and ("National" in inv.fiscal_position.name)):
                ...
                if not partner.is_company and inv.origin_type:
                    ...
                    ("Taxare Inversa" in inv.fiscal_position.name) or ("Comunitar" in inv.fiscal_position.name)
                ):
                    oper_type = "C"
    ...
    def _get_tax_ids(self):
        for inv in self:
            check = False
            taxes = []
            if not inv.fiscal_position or (
                ("National" in inv.fiscal_position.name)
                or ("Invers" in inv.fiscal_position.name)
    ...
    origin_type = fields.Selection(ORIGIN_TYPE, default="1", string="Origin Type")
    sequence_type = fields.Selection(SEQUENCE_TYPE, related="journal_id.sequence_type", string="Sequence Type")
    operation_type = fields.Selection(OPERATION_TYPE, compute="_get_operation_type", string="Operation Type")
    inv_serie = fields.Char("Invoice Serie", compute="_get_inv_number")
    inv_number = fields.Char("Invoice Number", compute="_get_inv_number")
    partner_type = fields.Char("D394 Partner Type", compute="_get_partner_type")
    special_taxes = fields.Boolean("Special Taxation")
    tax_ids = fields.Many2many("account.tax", compute="_get_tax_ids", string="Normal Taxes")
