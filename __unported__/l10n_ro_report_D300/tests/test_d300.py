...
class TestD300Report(TransactionCase):
    def setUp(self):
        ...
        self.invoice_2 = self.env.ref("l10n_ro_report_D300.demo_invoice_2")
        self.tag_ro_090 = self.env.ref("l10n_ro_report_D300.demo_tax_tag_ro_090")
        self.tag_ro_100 = self.env.ref("l10n_ro_report_D300.demo_tax_tag_ro_100")
        self.tag_ro_110 = self.env.ref("l10n_ro_report_D300.demo_tax_tag_ro_110")
        self.tag_ro_170 = self.env.ref("l10n_ro_report_D300.demo_tax_tag_ro_170")
        self.tax_ro_05 = self.env.ref("l10n_ro_report_D300.demo_tvac_05")
        self.tax_ro_05_cb = self.env.ref("l10n_ro_report_D300.demo_tvaic_05")
        self.bank_journal = self.env["account.journal"].search([("type", "=", "bank")], limit=1)

    def _get_report_lines(self):
        company = self.env.ref("base.main_company")
        self.invoice_2.pay_and_reconcile(self.bank_journal.id, 66.5, time.strftime("%Y-%m-10"))
        d300_report = self.env["l10n_ro_report_d300"].create(
            {"date_from": self.date_from, "date_to": self.date_to, "company_id": company.id, "tax_detail": True,}
        )
        ...
        lines["tag_ro_090"] = d300_taxtag_model.search(
            [("report_id", "=", d300_report.id), ("taxtag_id", "=", self.tag_ro_090.id),]
        )
        lines["tag_ro_100"] = d300_taxtag_model.search(
            [("report_id", "=", d300_report.id), ("taxtag_id", "=", self.tag_ro_100.id),]
        )
        lines["tag_ro_110"] = d300_taxtag_model.search(
            [("report_id", "=", d300_report.id), ("taxtag_id", "=", self.tag_ro_110.id),]
        )
        lines["tag_ro_170"] = d300_taxtag_model.search(
            [("report_id", "=", d300_report.id), ("taxtag_id", "=", self.tag_ro_170.id),]
        )
        d300_tax_model = self.env["l10n_ro_report_d300_tax"]
        lines["tax_05"] = d300_tax_model.search(
            [("report_tax_id", "=", lines["tag_ro_170"].id), ("tax_id", "=", self.tax_ro_05.id),]
        )
        lines["tax_05_cb"] = d300_tax_model.search(
            [("report_tax_id", "=", lines["tag_ro_170"].id), ("tax_id", "=", self.tax_ro_05_cb.id),]
        )
