            ...
            if self.vat_on_payment:
                if line_tup[2].get('account_id', False):
                    ...
                    if account.type != 'receivable' and account.type != 'payable':
                        ...
                        line_tup[2]['account_id'] = account.uneligible_account_id.id or account.id
                ...
                if line_tup[2].get('tax_code_id', False):
                    ...
                    line_tup[2]['tax_code_id'] =  tax_code.uneligible_tax_code_id.id or tax_code.id
    ...
    def _onchange_partner_id(self):
        ...
        for invoice in self:
            ...
            if 'out' in invoice.type:
                vat_on_payment = invoice.user_id.company_id.vat_on_payment
            else:
                if invoice.partner_id:
                    if invoice.partner_id.property_account_position:
                        if 'National' in invoice.partner_id.property_account_position.name:
                            vat_on_payment = invoice.partner_id.vat_on_payment
                    else:
                        vat_on_payment = invoice.partner_id.vat_on_payment
            invoice.vat_on_payment = vat_on_payment
        return result

    @api.multi
    @api.onchange('fiscal_receipt')
    def onchange_fiscal_receipt(self):
        for invoice in self:
            if invoice.fiscal_receipt:
                invoice.vat_on_payment = False
        return
