            ...
            if voucher.journal_id.type == "cash":
                ...
                if not bank_line:
                    if voucher.type in ("sale", "receipt"):
                        account_id = (
                            voucher.partner_id.property_account_receivable
                            and voucher.partner_id.property_account_receivable.id
                        )
                        if not account_id:
                            account_id = (
                                voucher.company_id.property_customer_advance_account_id
                                and voucher.company_id.property_customer_advance_account_id.id
                            )
                    else:
                        account_id = (
                            voucher.partner_id.property_account_payable
                            and voucher.partner_id.property_account_payable.id
                        )
                        if not account_id:
                            account_id = (
                                voucher.company_id.property_supplier_advance_account_id
                                and voucher.company_id.property_supplier_advance_account_id.id
                            )
                    ...
                    if statement.state != "open":
                        raise UserError(
                            ...
                            % (voucher.journal_id.name, voucher.date)
                        )
                    args = {
