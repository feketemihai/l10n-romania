...
class account_balance_romania(report_sxw.rml_parse, common_report_header):
    ...
    def __init__(self, cr, uid, name, context=None):
        super(account_balance_romania, self).__init__(cr, uid, name, context=context)
    ...
    def lines(self, form, ids=None, done=None):
        ...
        if form['filter'] == 'filter_period':
            ...
            ctx['period_from'] = period_obj.browse(self.cr, self.uid, period_obj.find(self.cr, self.uid, dt=False))[0].id
            ctx['period_to'] =  period_obj.browse(self.cr, self.uid, period_obj.find(self.cr, self.uid, dt=False))[0].id
        ctx['state'] = form['target_move']
        parents = ids
        child_ids = account_obj._get_children_and_consol(self.cr, self.uid, ids, ctx)
        if child_ids:
            ids = child_ids
        accounts = account_obj.read(self.cr, self.uid, ids, ['type','code','name','debit','credit','balance','parent_id','level','child_id'], ctx)
        ...
        ctx['state'] = form['target_move']
        ctx['fiscalyear'] = form['fiscalyear_id']
        ctx['periods'] = period_obj.search(self.cr, self.uid, [('fiscalyear_id','=',fiscalyear.id),('special','=',True),('date_start','=',fiscalyear.date_start)])
        ...
        if form['filter'] == 'filter_period':
            ...
            ctx['date_to'] = str((datetime.strptime(form['date_from'],'%Y-%m-%d') + timedelta(days=-1)).date())
        else:
            period_now = period_obj.browse(self.cr, self.uid, period_obj.find(self.cr, self.uid, dt=False))[0]
        
            ctx['date_from'] = str(fiscalyear.date_start)
            ctx['date_to'] =  str((datetime.strptime(period_now.date_start,'%Y-%m-%d') + timedelta(days=-1)).date())
