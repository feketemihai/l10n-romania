...
class d394_report(osv.osv_memory):
	...
	def _get_datas(self, cr, uid, ids, context=None):
		...
		function = email = ''
		if employee:
			employee = self.pool.get('hr.employee').browse(cr, uid, employee[0])
			if employee.job_id:
		...
		data_company = obj_company.browse(cr, uid, wiz_data.company_id.id, context=context)
			...
			if not inv.fiscal_receipt:
				...
				if part.vat and part.vat_subjected and ('RO' in part.vat.upper()):
					...
					if nrCUI > len(cui):
						cui[nrCUI] = part.id
		invoices = []			
		invoices = obj_invoice.browse(cr, uid, obj_invoice.search(cr, uid, [('state','in',['open','paid']),('type','in',['out_invoice','out_refund']),('period_id','=',period),('company_id','=',company)]))			
		for key in cui.iterkeys():
			in_xml = []
			tip = 'L'
			...
			if bazainv<>0:
				cer = {}			
				cereals = []
				codes = self.pool.get('report.394.code').search(cr, uid, [('name', 'in', ('10011000','10011900','10019110','10019120','10019900','1002','1003','1005','1201','1205','120600','121291'))])
				if codes:
					codes = [code394.id for code394 in self.pool.get('report.394.code').browse(cr, uid, codes)]
					for code in codes:
						cer[code] = {
									'baza': 0.0,
									'tva': 0.0,
							...
							if inv.fiscal_position and ('Taxare Inversa' in inv.fiscal_position.name):
								for line in inv.invoice_line:
									if line.product_id and line.product_id.d394_id:
										...
										if code in codes:
											cer[code]['baza'] += line.price_subtotal
											bazacer += line.price_subtotal
											if line.invoice_line_tax_id:
												taxes = tax_obj.compute_all(cr, uid, line.product_id.taxes_id, line.price_subtotal, 1, product=line.product_id, partner=line.invoice_id.partner_id)
												cer[code]['tva'] += taxes['total_included'] - taxes['total']
												tvacer += taxes['total_included'] - taxes['total']
		...
		invoices = []
		invoices = obj_invoice.browse(cr, uid, obj_invoice.search(cr, uid, [('state','in',['open','paid']),('type','=','in_invoice'),('period_id','=',period),('company_id','=',company)]))			
		for key in cui.iterkeys():
			in_xml = []
								...
								for line in inv.invoice_line:
									if line.product_id and line.product_id.d394_id:
										...
										if code in codes:
											cer[code]['baza'] += line.price_subtotal
											bazacer += line.price_subtotal
											taxes = tax_obj.compute_all(cr, uid, line.product_id.supplier_taxes_id, line.price_subtotal, 1, product=line.product_id, partner=line.invoice_id.partner_id)
											cer[code]['tva'] += taxes['total_included'] - taxes['total']
											tvacer += taxes['total_included'] - taxes['total']
					for key in cer.iterkeys():
						if cer[key]['baza']<>0:
...
<rezumat nrCui="%(nrCui)s" bazaL="%(bazaL)s" tvaL="%(tvaL)s" bazaA="%(bazaA)s" tvaA="%(tvaA)s" bazaV="%(bazaV)s" tvaV="%(tvaV)s" bazaVc="%(bazaVc)s" tvaVc="%(tvaVc)s"  bazaC="%(bazaC)s" tvaC="%(tvaC)s" bazaCc="%(bazaCc)s" tvaCc="%(tvaCc)s"/>""" % (xml_data)
	...
	<op1 tip="%s" cuiP="%s" denP="%s" baza="%s" tva="%s" >""" % ((client['tip']), (client['cuiP']), (client['denP']), (client['baza']), (client['tva']))
		...
		model_data_ids = mod_obj.search(cursor, user,[('model','=','ir.ui.view'),('name','=','view_d394_save')], context=context)
